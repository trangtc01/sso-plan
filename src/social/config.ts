...
export function loadYoutubePlaywrightConfig(env = process.env): YoutubePlaywrightConfig {
  ...
  return {
    profileDir: path.resolve(env.YOUTUBE_PROFILE_DIR ?? ".social-automation/youtube-profile"),
    artifactDir: path.resolve(env.YOUTUBE_ARTIFACT_DIR ?? ".social-automation/youtube-artifacts"),
    uploadUrl: env.YOUTUBE_UPLOAD_URL?.trim() || "https://www.youtube.com/upload",
    chromeExecutablePath: env.YOUTUBE_CHROME_EXECUTABLE?.trim() || undefined,
    chromeProfileDirectory: env.YOUTUBE_CHROME_PROFILE_DIRECTORY?.trim() || undefined,
