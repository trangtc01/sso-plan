...
## P0 — Historical Facebook credential exposure — rotation still required

A real Facebook Page access token was committed in `.env.example` history.

Action status:
- `.env.example` in repo has been sanitized with `FB_PAGE_ACCESS_TOKEN=""`.
- Rotation/revocation on Meta App Dashboard is pending user action.
- Audit confirmed no active credentials remain in tracked code files.
